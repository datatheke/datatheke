...
class CollectionReaderVoter implements VoterInterface
{
    const SUPPORTED_CLASS = 'Datatheke\\Bundle\\CoreBundle\\Document\\Collection';

    protected $voter;

    public function __construct(LibraryReaderVoter $voter)
    {
        $this->voter = $voter;
    }
    ...
    {
        return 'COLLECTION_READER' === strtoupper($attribute);
    }
    ...
    public function vote(TokenInterface $token, $collection, array $attributes)
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if (self::ACCESS_GRANTED === $this->voter->vote($token, $collection->getLibrary(), array('LIBRARY_READER'))) {
